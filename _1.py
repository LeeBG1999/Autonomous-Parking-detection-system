# Input:
#       /data/video/tran_dai_nghia_c9_10s.mp4
#Output:
#       /tracking_result/result.avi'
...
def main(_argv):
    ...
    while True:
        ...
        if len(dict_cars) == 0:
            ...
            bbox_color = utils.bgr2rgb((255,69,0))
            arrow_color = utils.bgr2rgb((0,0,128))
            line_color = utils.bgr2rgb(	(0,0,128))
            text_color = utils.bgr2rgb((0, 0, 0))
            colorListLine2 = utils.bgr2rgb((255, 0, 0))  # yellow
            colorListLine = utils.bgr2rgb((255, 255, 0))  # red
            # ----------------------------------------------------------------------------------------
            ...
            for track in tracker.tracks:
                ...
                if str(track.track_id) in dict_cars:
                    if get_iou(dict_cars[str(track.track_id)],
                        ...
                        #     # cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), bbox_color, 2)
                        #     cv2.line(frame, (int(bbox[0]+10), 360), (int(bbox[0]+10), 720), line_color, 2)
                        #     # cv2.putText(frame, str(int(bbox[3]-bbox[1])), (int(bbox[0]), int(bbox[1])+15), 0, 0.75, text_color, 1)
                        #     right_2 = [i for i in [int(bbox[0] + 10), 360, int(bbox[0] + 10), 720]]
                        #
                        #
                        # if int(track.track_id) == 2:
                        #     cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), bbox_color, 2)
                        #     cv2.line(frame, (int(bbox[0]), 360), (int(bbox[0]), 720), line_color, 2)
                        #     cv2.line(frame, (int(bbox[2])-2, 360), (int(bbox[2] - 2), 720),line_color, 2)
                        #     cv2.putText(frame, str(int(bbox[3] - bbox[1])), (int(bbox[0]), int(bbox[1]) + 20), 0, 0.75, text_color, 2)
                        #     left_2 = [i for i in [int(bbox[2]) - 2, 360, int(bbox[2] - 2), 720]]
                        #     right_1 = [i for i in [int(bbox[0]), 360, int(bbox[0]), 720]]
                        #
                        # # if int(track.track_id) == 3:
                        # #     cv2.line(frame, (int(bbox[2]) +130, 360), (int(bbox[2] + 130), 720), line_color, 2)
                        # #     left_1 = [i for i in [int(bbox[2]) +130, 360, int(bbox[2] + 130), 720]]
                        #
                        # if int(track.track_id) == 7:
                        #     cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), bbox_color, 2)
                        #     # cv2.line(frame, (int(bbox[0]), 360), (int(bbox[0]), 720), line_color, 2)
                        #     cv2.line(frame, (int(bbox[2])  , 360), (int(bbox[2] ), 720), line_color, 2)
                        #     cv2.putText(frame, str(int(bbox[3] - bbox[1])), (int(bbox[0]), int(bbox[1]) + 20), 0, 0.75,
                        #                 text_color, 2)
                        #     left_1 = [i for i in [int(bbox[2]), 360, int(bbox[2]-20), 720]]
                        #     dict_fix_car.pop(str(track.track_id))
                        #
                        # if int(track.track_id) == 8:
                        #     dict_fix_car.pop(str(track.track_id))
                    else:
            ...
            # # vẽ min space
            # cv2.arrowedLine(frame, (right_2[0]+50, right_2[1] +275),(right_2[0], right_2[1] +275),  arrow_color, 3)
            # cv2.arrowedLine(frame, (left_2[0]-50, left_2[1] +275), (left_2[0], left_2[1] +275), arrow_color, 3)
            # cv2.putText(frame, "Safe Margin", (right_2[0]+50, right_2[1] +275+25), 0, 0.75, text_color, 2)
            # cv2.putText(frame, str(int(right_2[0] - left_2[0])), (left_2[0]-50, left_2[1] +275+25), 0, 0.75,
            #             text_color, 2)
            # #Vẽ vacant space
            # cv2.arrowedLine(frame, (right_1[0], right_1[1] +280), (left_1[0], left_1[1] +280), arrow_color, 3,tipLength = 0.03)
            # cv2.arrowedLine(frame, (left_1[0], left_1[1] +280), (right_1[0], right_1[1] +280), arrow_color, 3,tipLength = 0.03)
            # cv2.putText(frame, "Vacant space", (left_1[0]+25, left_1[1] +280-10), 0, 0.75, text_color,2)
            # cv2.putText(frame, str(int(right_1[0] - left_1[0])), (left_1[0] + 85, left_1[1] + 280 + 25), 0, 0.75, text_color, 2)
    ...
    # ảnh trái thì bỏ comment, ko thif thôi
    ...
    cv2.imwrite("paper_image/intro_5.jpg",frame)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    f.close()
